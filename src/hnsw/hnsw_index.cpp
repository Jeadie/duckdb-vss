...
unique_ptr<IndexScanState> HNSWIndex::InitializeScan(float *query_vector, idx_t limit, ClientContext &context,
	...
	if (bitmap) {
		// Use exact filtered search: scan all nodes with the bitmap predicate.
		// exact=true is required for correctness because the approximate HNSW graph traversal
		// may terminate early (radius shrinks based on farthest result found so far) and miss
		// filtered results beyond that radius. Since the bitmap build is already O(n), the
		// total query cost remains O(n) and exact search guarantees we return `limit` results
...
idx_t HNSWIndex::ExecuteMultiScan(IndexScanState &state_p, float *query_vector, idx_t limit) {
	...
	USearchIndexType::search_result_t search_result;
	{
		...
		if (state.filter_bitmap) {
			...
			// exact=true: see comment in InitializeScan for rationale
			search_result = index.filtered_ef_search(
